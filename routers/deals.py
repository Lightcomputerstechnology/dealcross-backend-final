...
from utils.notifications import send_email, create_notification
...
@router.post("/{deal_id}/fund", summary="Fund a deal and notify counterparty")
def fund_deal(
    deal_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
...
@router.post("/{deal_id}/deliver", summary="Mark a deal as delivered and notify creator")
def mark_delivered(
    deal_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
...
@router.post("/{deal_id}/dispute", summary="Raise a dispute on a deal and notify both parties")
def raise_dispute(
    deal_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
