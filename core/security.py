...
# ───────────────── Legacy JWT helpers (HS256 with local secret) ─────────────────
...
def decode_supabase_token(token: str) -> dict:
    ...
    try:
        ...
        payload = jwt.decode(
            token,
            public_key,               # python-jose accepts a JWK dict
            algorithms=[alg],
            options={"verify_aud": False},  # set to True & pass audience if you enforce aud
        )

        # Defensive exp check (jose also checks if 'exp' present)
        exp = payload.get("exp")
...
async def get_current_user(token: str = Depends(oauth2_scheme)):
    ...
    Fallback: verify legacy HS256 token with local secret.
    ...
    if getattr(settings, "SUPABASE_JWKS_URL", None):
        try:
            ...
            claims.setdefault(
                "email",
                claims.get("email") or (claims.get("user_metadata") or {}).get("email"),
            )
