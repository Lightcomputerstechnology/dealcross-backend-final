...
revision = '20250423_1245_3c1a2b4e6d7f' down_revision = None branch_labels = None depends_on = None

def upgrade(): # Users table op.create_table( 'users', sa.Column('id', sa.Integer, primary_key=True), sa.Column('email', sa.String, unique=True, nullable=False), sa.Column('hashed_password', sa.String, nullable=False), sa.Column('role', sa.String, nullable=False, default='user'), sa.Column('created_at', sa.DateTime, server_default=sa.func.now()), )
...
def downgrade(): op.drop_table('audit_logs') op.drop_table('fraud_alerts') op.drop_table('disputes') op.drop_table('kyc') op.drop_table('deals') op.drop_table('wallets') op.drop_table('users')
